...
export default function HoDLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // OPTIONAL: verify role after login
      const tokenResult = await auth.currentUser?.getIdTokenResult();
      if (tokenResult?.claims?.role !== "hod") {
        setError("Access denied. Not an HoD account.");
        return;
      }
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white shadow-xl rounded-2xl p-8 w-full max-w-md border border-gray-100">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2 text-sm uppercase tracking-wider font-semibold">Project Review & Approval</p>
        </div>

        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="hod-email" className="text-xs font-bold text-gray-600 uppercase tracking-tight ml-1">HoD Email</label>
            <input
              id="hod-email"
              type="email"
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="hod-password" className="text-xs font-bold text-gray-600 uppercase tracking-tight ml-1">Password</label>
